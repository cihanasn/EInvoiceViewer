...
";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                // Kullanıcının kayıt dizinini seçmesini sağla
                using (FolderBrowserDialog folderDialog = new FolderBrowserDialog())
                {
                    folderDialog.Description = "PDF dosyasını kaydetmek için bir klasör seçin.";
                    if (folderDialog.ShowDialog() == DialogResult.OK)
                    {
                        string selectedFolder = folderDialog.SelectedPath;

                        // Devamında bu klasöre kaydedilsin
                        string pdfOutputPath = Path.Combine(selectedFolder, $"fatura_preview_{Guid.NewGuid()}.pdf");

                        // Devam eden kodlarda downloadsPath yerine pdfOutputPath kullanabilirsiniz
                    ...
                    {
                        MessageBox.Show("Kayıt dizini seçilmedi.");
                        return;
                    }
                }
